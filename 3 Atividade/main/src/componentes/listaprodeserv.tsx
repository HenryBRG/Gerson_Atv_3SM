...
const ListaProdutosServicos: React.FC<Props> = ({ tema }) => {
  ...
  <div className="list-group">
    {/* Exibe a lista de produtos cadastrados, caso existam */}
    {produtos.length > 0 && (
      <div>
        <h3>Produtos</h3>
        {produtos.map((produto, index) => (
          <div key={index}>
            {/* Nome do produto que pode ser clicado para expandir informações */}
            <a
              href="#"
            ...
            {/* Conteúdo expandido do produto */}
            {expandedProdutoIndex === index && (
              <div className="dropdown-content">
                {/* Botões de ação para deletar ou atualizar o produto */}
                {editIndex !== index && (
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    <button
                      ...
                      onClick={() => deleteProduto(produto.nomeprod)}
                    >
                      Deletar
                    </button>
                    <button
                      className="btn btn-info"
                      onClick={() => enableEditProduto(produto, index)}
                    >
                      Atualizar
                    </button>
                  </div>
                ...
                {/* Formulário de edição para o produto selecionado */}
                {editIndex === index && editingProduto ? (
                  <div>
                    {/* Campo para edição do nome do produto */}
                    <input
                      type="text"
                      className="inputscadastroproduto2"
                      value={editingProduto.nomeprod}
                      onChange={(e) =>
                        handleChangeProduto("nomeprod", e.target.value)
                      }
                    />
                    {/* Campo para edição do valor do produto */}
                    <input
                      type="number"
                      className="inputscadastroproduto2"
                      value={editingProduto.valorprod}
                      onChange={(e) =>
                        handleChangeProduto("valorprod", e.target.value)
                      }
                    />
                    {/* Campo para edição do tipo do produto */}
                    <input
                      type="text"
                      className="inputscadastroproduto2"
                      value={editingProduto.tipoprod}
                      onChange={(e) =>
                        handleChangeProduto("tipoprod", e.target.value)
                      }
                    />
                    {/* Campo para edição da descrição do produto */}
                    <input
                      type="text"
                      className="inputscadastroproduto2"
                      value={editingProduto.descricaoprod}
                      onChange={(e) =>
                        handleChangeProduto("descricaoprod", e.target.value)
                      }
                    />
                    {/* Botão para salvar as alterações feitas no produto */}
                    <button
                      className="btn btn-success"
                      onClick={saveEditProduto}
                    >
                      Salvar
                    </button>
                  </div>
                ) : (
                  /* Informações do produto exibidas em modo de visualização */
                  <div>
                    <p>
                      <strong>Valor:</strong> R${produto.valorprod}
                    </p>
                    <p>
                      <strong>Tipo:</strong> {produto.tipoprod}
                    </p>
                    <p>
                      <strong>Descrição:</strong> {produto.descricaoprod}
                    </p>
                  </div>
                )}
              </div>
